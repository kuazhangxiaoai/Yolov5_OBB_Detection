...
def rbox_iou(box1, theta1, box2, theta2):
    ...
    for i in range(len(polys1)):
        ...
        iou = polyiou.poly_iou(polys1[i], polys2[i])
        IoUs.append(iou)
    IoUs = np.array(IoUs)
...
def py_cpu_nms_poly_fast(dets, scores, thresh):
    ...
    for i in range(len(dets)):
        tm_polygon = [dets[i][0], dets[i][1],dets[i][2], dets[i][3],dets[i][4], dets[i][5],dets[i][6], dets[i][7]]
    ...
    while order.size > 0:
        ...
        for j in range(tmp_order.size):
            iou = polyiou.poly_iou(polys[i], polys[tmp_order[j]])
